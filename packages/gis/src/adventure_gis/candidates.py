...
def _min_dist_km(origin: Point, places: Iterable[NamedPoint]) -> float:
...
def generate_candidates(
    ...
    *,
    ...
    for seed in pack.catalog:
        ...
        relief = float(seed.properties.get("relief_m") or 0.0)
        if relief <= 0:
            relief = _local_relief_m(origin, pack.elevation_samples)
        elev = float(seed.properties.get("elevation_m", 0))
        ...
        building_density = float(seed.properties.get("building_density", 0.1))
        crowd_prop = float(seed.properties.get("crowd", building_density))
        novelty = _clamp01(remoteness * 0.7 + (1.0 - building_density) * 0.3)
        crowd = _clamp01(crowd_prop)

        forest_prop = float(seed.properties.get("forest", 0.0))
        if "forest" in tags:
            forest_prop = max(forest_prop, 0.75)
        forest = _clamp01(forest_prop)

        access = _access_fit(dist_road, vehicle, vehicle_class, days)
        camping = _clamp01(
            (1.0 - float(seed.properties.get("slope", 0.3))) * 0.6 + water * 0.25 + access * 0.15
        )
        risk = _clamp01(float(seed.properties.get("hazard", 0.2)) + (0.3 if relief > 1000 else 0.0))
        restriction = _clamp01(float(seed.properties.get("protected", 0.0)))
        ...
        features = CandidateFeatures(
            ...
            dist_settlement_km=round(dist_settlement, 2),
            dist_road_km=round(dist_road, 2),
            dist_water_km=round(dist_water, 2),
            elevation_m=elev,
